#include "cnode.h"
#include "color.h"
#include <arpa/inet.h>   /* inet_ntop for IPv6 */
...
// print IPv6 address from 16-byte raw array
static void print_ipv6(const uint8_t *addr)
{
		...
		uint16_t count, int32_t range, int depth)
{
	...
	if (count > ED) {
		printf(" ... (%u more)", (unsigned int)(count - ED));
	}
	printf(" ]\n");
}

/* Print a uint32_t array with a limit on displayed elements. */
...
/* Print an IP address array, supporting base+N range display. */
...
/* Dump the full Cnode configuration tree to stdout for debugging.
 *
 * Recursively prints every field: ether, IPv4, TCP, UDP, ICMP, ARP,
 * VXLAN, erroneous, offload flags, etc. */
void cnode_show(const Cnode *node, int depth)
{
	...
	/* IMIX */
	if (node->ether.n_imix) {
		print_indent(depth);
		printf("imix[%u]: [ ", node->ether.n_imix);
		for (uint16_t i = 0; i < node->ether.n_imix && i < ED; i++) {
			printf("%u%s", node->ether.imix[i],
				(i < node->ether.n_imix - 1) ? ", " : "");
		}
		if (node->ether.n_imix > ED) {
			printf(" ...");
		}
		printf(" ]\n");
	}
	...
	print_indent(depth);
	printf("n_src: %u, n_dst: %u\n",
		node->ether.type.ipv6.n_src,
		node->ether.type.ipv6.n_dst);

	if (node->ether.type.ipv6.n_src) {
		print_indent(depth);
		printf("src[%u]: [ ", node->ether.type.ipv6.n_src);
		for (uint16_t i = 0; i < node->ether.type.ipv6.n_src
			&& i < ED; i++) {
			print_ipv6(node->ether.type.ipv6.src[i]);
			if (i < node->ether.type.ipv6.n_src - 1) {
				printf(", ");
			}
		}
		if (node->ether.type.ipv6.n_src > ED) {
			printf(" ... (%u more)",
			       (unsigned int)(node->ether.type.ipv6.n_src - ED));
		}
		printf(" ]\n");
	}
	if (node->ether.type.ipv6.n_dst) {
		print_indent(depth);
		printf("dst[%u]: [ ", node->ether.type.ipv6.n_dst);
		for (uint16_t i = 0; i < node->ether.type.ipv6.n_dst
			&& i < ED; i++) {
			print_ipv6(node->ether.type.ipv6.dst[i]);
			if (i < node->ether.type.ipv6.n_dst - 1) {
				printf(", ");
			}
		}
		if (node->ether.type.ipv6.n_dst > ED) {
			printf(" ... (%u more)",
			       (unsigned int)(node->ether.type.ipv6.n_dst - ED));
		}
		printf(" ]\n");
	}
	...
	printf("tcp: {\n");
	depth++;
	print_array_u16("src", node->ether.type.ipv6.tcp.src,
		node->ether.type.ipv6.tcp.n_src,
		node->ether.type.ipv6.tcp.src_range, depth);
	print_array_u16("dst", node->ether.type.ipv6.tcp.dst,
		node->ether.type.ipv6.tcp.n_dst,
		node->ether.type.ipv6.tcp.dst_range, depth);
	print_indent(depth);
	printf("src_range: %d\n", node->ether.type.ipv6.tcp.src_range);
	print_indent(depth);
	printf("dst_range: %d\n", node->ether.type.ipv6.tcp.dst_range);
	depth--;
	print_indent(depth);
	printf("}\n");
	...
	printf("udp: {\n");
	depth++;
	print_array_u16("src", node->ether.type.ipv6.udp.src,
		node->ether.type.ipv6.udp.n_src,
		node->ether.type.ipv6.udp.src_range, depth);
	print_array_u16("dst", node->ether.type.ipv6.udp.dst,
		node->ether.type.ipv6.udp.n_dst,
		node->ether.type.ipv6.udp.dst_range, depth);
	print_indent(depth);
	printf("src_range: %d\n", node->ether.type.ipv6.udp.src_range);
	print_indent(depth);
	printf("dst_range: %d\n", node->ether.type.ipv6.udp.dst_range);
	depth--;
	print_indent(depth);
	printf("}\n");
	...
	for (uint16_t i = 0; i < node->erroneous.n_clas; i++) {
		...
		if (node->erroneous.clas[i].n_type > ED) {
			printf(" ... (%u more)", (unsigned int)(node->erroneous.clas[i].n_type - ED));
		}
		printf(" ]\n");
	...
	/* Extension slots */	if (node->n_ext) {
		print_indent(depth);
		printf("\"ext\": {\n");
		for (uint8_t i = 0; i < node->n_ext; i++) {
			print_indent(depth + 1);
			printf("\"%s\": ", node->ext[i].desc->name);
			if (node->ext[i].desc->show_cfg) {
				node->ext[i].desc->show_cfg(node->ext[i].cfg, depth + 1);
			} else {
				printf("(%zu bytes)\n", node->ext[i].desc->cfg_size);
			}
		}
		print_indent(depth);
		printf("}\n");
...
// ============================================
// Simplified dump (key fields only)
// ============================================
void cnode_show_summary(const Cnode *node)
{
	...
	printf("\nEther:\n");
	printf("  mtu: %u, copy_payload: %u, imix: %u\n",
			node->ether.mtu, node->ether.copy_payload, node->ether.n_imix);
	printf("  dst MAC: ");
	print_mac(node->ether.dst);
	printf("\n  src MAC: ");
	print_mac(node->ether.src);
	...
	printf("  IPv6: %u src, %u dst\n",
			node->ether.type.ipv6.n_src, node->ether.type.ipv6.n_dst);
	...
	printf("    TCP: %u src ports, %u dst ports, payload_len: %u\n",
			(unsigned int)(node->ether.type.ipv4.tcp.n_src ?
			node->ether.type.ipv4.tcp.n_src :
			abs(node->ether.type.ipv4.tcp.src_range)),
			(unsigned int)(node->ether.type.ipv4.tcp.n_dst ?
			node->ether.type.ipv4.tcp.n_dst :
			abs(node->ether.type.ipv4.tcp.dst_range)),
			node->ether.type.ipv4.tcp.payload_len);
	printf("    UDP: %u src ports, %u dst ports, payload_len: %u\n",
			(unsigned int)(node->ether.type.ipv4.udp.n_src ?
			node->ether.type.ipv4.udp.n_src :
			abs(node->ether.type.ipv4.udp.src_range)),
			(unsigned int)(node->ether.type.ipv4.udp.n_dst ?
			node->ether.type.ipv4.udp.n_dst :
			abs(node->ether.type.ipv4.udp.dst_range)),
			node->ether.type.ipv4.udp.payload_len);

	printf("\nVXLAN:\n");
	printf("  enable: %u, ratio: %u, wire_mtu: %u\n",
			node->vxlan.enable, node->vxlan.ratio, node->vxlan.wire_mtu);
	...
	/* Extension slots */	if (node->n_ext) {
		printf("\nExtensions (%u loaded):\n", node->n_ext);
		for (uint8_t i = 0; i < node->n_ext; i++) {
			const struct bless_ext_cfg *d = node->ext[i].desc;
			printf("  [%u] %s (%zu bytes)\n", i, d->name, d->cfg_size);
		}
	}
}
