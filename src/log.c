...
/**
 * @file log.c
 * @brief Structured / coloured logging with syslog fallback.
 *
 * LOG_* macros resolve to this module.  Supports per-level prefix
 * colours, timestamps, and optional syslog output in daemon mode.
 */
...
static const theme_config THEMES[] = {
	...
	{
		.name   = "Vibrant",
		.green  = C_VIBRANT_INFO,
		.yellow = C_VIBRANT_WARN,
		.red    = C_VIBRANT_ERROR,
		.blue   = C_VIBRANT_DEBUG,
		.purple = C_VIBRANT_TRACE,
		.grey   = C_VIBRANT_META,
	},
	{
		.name   = "Warm",
...
int g_log_format_json = 0;

void log_set_format_json(int enabled)
{
	g_log_format_json = enabled;
}
...
/* Initialise the logging system with a named colour theme.
 * The theme defines ANSI colour codes used by LOG_* macros. */
